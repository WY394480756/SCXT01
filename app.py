...
def add_cash_flow(amount, flow_type, account_type, related_id, note, created_by):
    ...
    current = get_setting(f'current_{account_type}')
    new_balance = current + amount
    update_setting(f'current_{account_type}', new_balance)
    conn.commit()
    conn.close()
# ---------- 页面路由 ----------
@app.route('/')
...
def checkout():
    ...
    try:
        ...
        c.execute("INSERT INTO sales (sale_time, total_amount, member_id, payment_method, created_by) VALUES (?, ?, ?, ?, ?)",
                  (sale_time, total_amount, member_id, payment_method, current_user.id))
        sale_id = c.lastrowid
...
def create_purchase():
    ...
    try:
        ...
        c.execute("INSERT INTO purchases (purchase_time,supplier,total_amount,payment_method,payment_status,created_by) VALUES (?,?,?,?,?,?)",
                  (pt,supplier,total,payment_method,'paid',current_user.id))
        pid = c.lastrowid
        for it in items:
            pid_int = it['product_id']
            qty = it['quantity']
            cost = it['unit_cost']
            c.execute("INSERT INTO purchase_items (purchase_id,product_id,quantity,unit_cost) VALUES (?,?,?,?)",
                      (pid,pid_int,qty,cost))
            c.execute("SELECT stock,cost FROM products WHERE id=? FOR UPDATE",(pid_int,))
            p = c.fetchone()
            new_stock = p['stock'] + qty
            new_cost = ((p['stock']*p['cost']) + (qty*cost)) / new_stock if new_stock>0 else 0
            c.execute("UPDATE products SET stock=?, cost=? WHERE id=?",(new_stock,new_cost,pid_int))
...
def get_purchases():
    conn,c = get_db()
    c.execute("SELECT id,purchase_time,supplier,total_amount,payment_method FROM purchases ORDER BY purchase_time DESC")
    rows = [dict(row) for row in c.fetchall()]
    conn.close()
    return jsonify(rows)
...
def get_cash_flows():
    conn,c = get_db()
    c.execute("SELECT * FROM cash_flows ORDER BY flow_time DESC")
    rows = [dict(row) for row in c.fetchall()]
    conn.close()
    return jsonify(rows)
...
def get_sales():
    conn,c = get_db()
    c.execute('''SELECT s.id,s.sale_time,s.total_amount,s.member_id,s.payment_method,u.username as created_by_name
...
def get_sale_detail(sid):
    conn,c = get_db()
    c.execute("SELECT id,sale_time,total_amount,member_id,payment_method FROM sales WHERE id=?",(sid,))
    sale = c.fetchone()
    if not sale:
...
def financial_report():
    ...
    current_cash = get_setting('current_cash')
    current_bank = get_setting('current_bank')
...
def export_sales_csv():
    conn,c = get_db()
    c.execute('''SELECT s.id,s.sale_time,s.total_amount,u.username as operator,m.name as member_name,s.payment_method
                 FROM sales s LEFT JOIN users u ON s.created_by=u.id LEFT JOIN members m ON s.member_id=m.id
                 ORDER BY s.sale_time DESC''')
    rows = c.fetchall()
    conn.close()
    output = StringIO()
    writer = csv.writer(output)
    writer.writerow(['销售单号','销售时间','总金额(元)','操作员','会员','支付方式'])
    for r in rows:
        writer.writerow([r['id'], r['sale_time'], r['total_amount'], r['operator'] or '', r['member_name'] or '散客', r['payment_method']])
...
<!DOCTYPE html>
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>登录</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet"></head>
...
<!DOCTYPE html>
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>用户管理</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet"></head>
<body>
<nav class="navbar navbar-expand-lg navbar-dark bg-dark"><div class="container"><a class="navbar-brand" href="/pos">商超系统</a><div class="navbar-nav"><a class="nav-link" href="/pos">收银台</a><a class="nav-link" href="/inventory">库存管理</a><a class="nav-link" href="/sales">销售记录</a><a class="nav-link" href="/members">会员管理</a><a class="nav-link" href="/purchase">采购入库</a><a class="nav-link" href="/finance">财务期初</a><a class="nav-link" href="/backup">数据备份</a><a class="nav-link active" href="/useradmin">用户管理</a><a class="nav-link" href="/logout">退出</a></div></div></nav>
...
<!DOCTYPE html>
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>收银台</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet">
<style>.cart-item{border-bottom:1px solid #eee;padding:8px 0}.low-stock{color:#d9534f;font-size:0.8rem}</style></head>
<body>
<nav class="navbar navbar-expand-lg navbar-dark bg-dark"><div class="container"><a class="navbar-brand" href="/pos">商超系统</a><div class="navbar-nav"><a class="nav-link active" href="/pos">收银台</a><a class="nav-link" href="/inventory">库存管理</a><a class="nav-link" href="/sales">销售记录</a><a class="nav-link" href="/members">会员管理</a><a class="nav-link" href="/purchase">采购入库</a><a class="nav-link" href="/finance">财务期初</a><a class="nav-link" href="/backup">数据备份</a><a class="nav-link" href="/useradmin">用户管理</a><a class="nav-link" href="/logout">退出</a></div><div class="navbar-nav ms-auto"><button class="btn btn-outline-light btn-sm me-2" onclick="window.print()">🖨️ 打印本页</button></div></div></nav>
<div class="container mt-4"><div class="row"><div class="col-md-7"><h3>商品列表</h3><input type="text" id="searchInput" class="form-control mb-3" placeholder="搜索..."><div id="productList" class="row"></div></div><div class="col-md-5"><h3>购物车 <button class="btn btn-sm btn-danger" id="clearCartBtn">清空</button></h3><div id="cartItems"></div><div class="mt-3"><div class="mb-2"><label>支付方式</label><select id="paymentMethod" class="form-select"><option value="cash">现金</option><option value="member_card">会员卡</option></select></div><div id="memberSelectDiv" style="display:none"><label>会员ID</label><input type="number" id="memberId" class="form-control" placeholder="输入会员ID"></div><h4>总计: ¥<span id="cartTotal">0.00</span></h4><button class="btn btn-success w-100" id="checkoutBtn">结算</button></div></div></div></div>
<script>
async function loadProducts(){ const resp=await fetch('/api/products'); const ps=await resp.json(); const search=document.getElementById('searchInput').value.toLowerCase(); const filtered=ps.filter(p=>p.name.toLowerCase().includes(search)); document.getElementById('productList').innerHTML=filtered.map(p=>`<div class="col-sm-6 col-md-4 mb-3"><div class="card"><div class="card-body"><h5>${escapeHtml(p.name)}</h5><p>¥${p.price.toFixed(2)}<br>库存:${p.stock} ${p.stock<10?'<span class="low-stock">(低库存)</span>':''}</p><div class="input-group"><input type="number" id="qty_${p.id}" class="form-control" value="1" min="1" max="${p.stock}"><button class="btn btn-primary" onclick="addToCart(${p.id})">加入</button></div></div></div></div>`).join(''); }
function escapeHtml(s){ return s.replace(/[&<>]/g,m=>m==='&'?'&amp;':m==='<'?'&lt;':'&gt;'); }
...
document.getElementById('checkoutBtn').onclick=async()=>{ let member_id=null, pm=document.getElementById('paymentMethod').value; if(pm==='member_card'){ member_id=parseInt(document.getElementById('memberId').value); if(isNaN(member_id)){ showToast('请输入会员ID','danger'); return; } } const resp=await fetch('/api/checkout',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({payment_method:pm,member_id:member_id})}); const result=await resp.json(); if(resp.ok){ showToast(`结算成功！金额:¥${result.total_amount}`,'success'); loadCart(); loadProducts(); if(result.ticket) printTicket(result.ticket); }else{ showToast(result.error,'danger'); } };
function printTicket(ticket){ let html=`<html><head><title>销售小票</title><style>body{font-family:monospace;padding:20px}table{width:100%}th,td{padding:4px}</style></head><body><h3>商超收银系统</h3><p>单号:${ticket.sale_id}<br>时间:${ticket.sale_time}<br>操作员:${ticket.operator}<br>会员:${ticket.member||'散客'}</p><table border="1" cellpadding="4"><tr><th>商品</th><th>单价</th><th>数量</th><th>小计</th></tr>${ticket.items.map(i=>`<tr><td>${escapeHtml(i.name)}</td><td>¥${i.price.toFixed(2)}</td><td>${i.quantity}</td><td>¥${(i.price*i.quantity).toFixed(2)}</td></tr>`).join('')}<tr><td colspan="3" align="right"><strong>总计</strong></td><td>¥${ticket.total.toFixed(2)}</td></tr></table><p>支付方式:${ticket.payment_method==='cash'?'现金':'会员卡'}</p><button onclick="window.print()">打印</button><script>window.onload=function(){setTimeout(function(){window.print();window.close();},500)}<\/script></body></html>`; let w=window.open('','_blank'); w.document.write(html); w.document.close(); }
...
<!DOCTYPE html>
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>库存管理</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet"></head>
<body>
<nav class="navbar navbar-expand-lg navbar-dark bg-dark"><div class="container"><a class="navbar-brand" href="/pos">商超系统</a><div class="navbar-nav"><a class="nav-link" href="/pos">收银台</a><a class="nav-link active" href="/inventory">库存管理</a><a class="nav-link" href="/sales">销售记录</a><a class="nav-link" href="/members">会员管理</a><a class="nav-link" href="/purchase">采购入库</a><a class="nav-link" href="/finance">财务期初</a><a class="nav-link" href="/backup">数据备份</a><a class="nav-link" href="/useradmin">用户管理</a><a class="nav-link" href="/logout">退出</a></div><div class="navbar-nav ms-auto"><button class="btn btn-outline-light btn-sm me-2" onclick="window.print()">🖨️ 打印本页</button><button class="btn btn-outline-light btn-sm" onclick="exportInventory()">📎 导出 CSV</button></div></div></nav>
...
<table class="table table-bordered"><thead><tr><th>ID</th><th>名称</th><th>售价</th><th>成本</th><th>库存</th><th>状态</th><th>操作</th></tr></thead><tbody id="productTableBody"></tbody></table></div>
...
<!DOCTYPE html>
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>销售记录</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet"></head>
<body>
<nav class="navbar navbar-expand-lg navbar-dark bg-dark"><div class="container"><a class="navbar-brand" href="/pos">商超系统</a><div class="navbar-nav"><a class="nav-link" href="/pos">收银台</a><a class="nav-link" href="/inventory">库存管理</a><a class="nav-link active" href="/sales">销售记录</a><a class="nav-link" href="/members">会员管理</a><a class="nav-link" href="/purchase">采购入库</a><a class="nav-link" href="/finance">财务期初</a><a class="nav-link" href="/backup">数据备份</a><a class="nav-link" href="/useradmin">用户管理</a><a class="nav-link" href="/logout">退出</a></div><div class="navbar-nav ms-auto"><button class="btn btn-outline-light btn-sm me-2" onclick="window.print()">🖨️ 打印本页</button><button class="btn btn-outline-light btn-sm" onclick="exportSales()">📎 导出 CSV</button></div></div></nav>
<div class="container mt-4"><h3>销售历史记录</h3><table class="table table-hover"><thead><tr><th>单号</th><th>时间</th><th>金额</th><th>会员</th><th>支付方式</th><th>操作员</th><th>操作</th></tr></thead><tbody id="salesTableBody"></tbody></table></div>
...
<script>
async function loadSales(){ const resp=await fetch('/api/sales'); const sales=await resp.json(); const tbody=document.getElementById('salesTableBody'); tbody.innerHTML=sales.map(s=>`<tr><td>${s.id}</td><td>${s.sale_time}</td><td>${s.total_amount.toFixed(2)}</td><td>${s.member_name||'散客'}</td><td>${s.payment_method==='cash'?'现金':'会员卡'}</td><td>${s.created_by_name||''}</td><td><button class="btn btn-sm btn-info" onclick="viewDetail(${s.id})">详情</button></td></tr>`).join(''); }
async function viewDetail(id){ const resp=await fetch(`/api/sales/${id}`); const d=await resp.json(); if(resp.ok){ let itemsHtml=d.items.map(i=>`<tr><td>${escapeHtml(i.name)}</td><td>${i.quantity}</td><td>${i.price_at_sale.toFixed(2)}</td><td>${i.subtotal.toFixed(2)}</td></tr>`).join(''); document.getElementById('detailContent').innerHTML=`<p>单号:${d.id}</p><p>时间:${d.sale_time}</p><p>总金额:${d.total_amount}</p><p>会员:${d.member_name||'无'}</p><table class="table table-sm"><thead><tr><th>商品</th><th>数量</th><th>单价</th><th>小计</th></tr></thead><tbody>${itemsHtml}</tbody></table>`; new bootstrap.Modal(document.getElementById('detailModal')).show(); } else alert('加载失败'); }
function escapeHtml(s){ return s.replace(/[&<>]/g,m=>m==='&'?'&amp;':m==='<'?'&lt;':'&gt;'); }
...
<!DOCTYPE html>
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>会员管理</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet"></head>
<body>
<nav class="navbar navbar-expand-lg navbar-dark bg-dark"><div class="container"><a class="navbar-brand" href="/pos">商超系统</a><div class="navbar-nav"><a class="nav-link" href="/pos">收银台</a><a class="nav-link" href="/inventory">库存管理</a><a class="nav-link" href="/sales">销售记录</a><a class="nav-link active" href="/members">会员管理</a><a class="nav-link" href="/purchase">采购入库</a><a class="nav-link" href="/finance">财务期初</a><a class="nav-link" href="/backup">数据备份</a><a class="nav-link" href="/useradmin">用户管理</a><a class="nav-link" href="/logout">退出</a></div><div class="navbar-nav ms-auto"><button class="btn btn-outline-light btn-sm me-2" onclick="window.print()">🖨️ 打印本页</button><button class="btn btn-outline-light btn-sm" onclick="exportMembers()">📎 导出 CSV</button></div></div></nav>
...
<table class="table table-bordered"><thead><tr><th>ID</th><th>姓名</th><th>手机</th><th>卡类型</th><th>余额/次数</th><th>有效期</th><th>操作</th></tr></thead><tbody id="memberTableBody"></tbody></table></div>
<!-- 会员模态框 -->
...
<!DOCTYPE html>
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>采购入库</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet"></head>
<body>
<nav class="navbar navbar-expand-lg navbar-dark bg-dark"><div class="container"><a class="navbar-brand" href="/pos">商超系统</a><div class="navbar-nav"><a class="nav-link" href="/pos">收银台</a><a class="nav-link" href="/inventory">库存管理</a><a class="nav-link" href="/sales">销售记录</a><a class="nav-link" href="/members">会员管理</a><a class="nav-link active" href="/purchase">采购入库</a><a class="nav-link" href="/finance">财务期初</a><a class="nav-link" href="/backup">数据备份</a><a class="nav-link" href="/useradmin">用户管理</a><a class="nav-link" href="/logout">退出</a></div><div class="navbar-nav ms-auto"><button class="btn btn-outline-light btn-sm me-2" onclick="window.print()">🖨️ 打印本页</button></div></div></nav>
<div class="container mt-4"><h3>采购入库单</h3><div class="row"><div class="col-md-6"><div><label>供应商</label><input id="supplier" class="form-control"></div><div class="mt-2"><label>支付方式</label><select id="payMethod" class="form-select"><option value="cash">现金</option><option value="bank">银行</option></select></div></div><div class="col-md-6"><h5>采购明细</h5><table class="table table-sm"><thead><tr><th>商品</th><th>数量</th><th>进价</th><th>小计</th><th></th></tr></thead><tbody id="purchaseItemsBody"></tbody></table><button class="btn btn-sm btn-primary" onclick="addPurchaseRow()">添加商品</button><button class="btn btn-success mt-3" id="submitPurchase">提交入库</button></div></div></div>
<script>
let itemIndex=0; let products=[];
async function loadProductsForPurchase(){ const resp=await fetch('/api/products'); const ps=await resp.json(); products=ps; }
...
<!DOCTYPE html>
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>财务期初</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet"></head>
<body>
<nav class="navbar navbar-expand-lg navbar-dark bg-dark"><div class="container"><a class="navbar-brand" href="/pos">商超系统</a><div class="navbar-nav"><a class="nav-link" href="/pos">收银台</a><a class="nav-link" href="/inventory">库存管理</a><a class="nav-link" href="/sales">销售记录</a><a class="nav-link" href="/members">会员管理</a><a class="nav-link" href="/purchase">采购入库</a><a class="nav-link active" href="/finance">财务期初</a><a class="nav-link" href="/backup">数据备份</a><a class="nav-link" href="/useradmin">用户管理</a><a class="nav-link" href="/logout">退出</a></div><div class="navbar-nav ms-auto"><button class="btn btn-outline-light btn-sm me-2" onclick="window.print()">🖨️ 打印本页</button></div></div></nav>
<div class="container mt-4"><div class="row"><div class="col-md-6"><div class="card"><div class="card-body"><h5>期初余额设置</h5><div class="mb-2"><label>现金</label><input id="cash" type="number" step="0.01" class="form-control"></div><div class="mb-2"><label>银行存款</label><input id="bank" type="number" step="0.01" class="form-control"></div><button id="saveInitBtn" class="btn btn-primary">保存期初</button></div></div></div><div class="col-md-6"><div class="card"><div class="card-body"><h5>财务报表</h5><p>销售收入: ¥<span id="totalSales">0</span></p><p>销售成本: ¥<span id="totalCost">0</span></p><p>毛利: ¥<span id="grossProfit">0</span></p><p>采购总额: ¥<span id="purchasesTotal">0</span></p><hr><p>现金余额: ¥<span id="balanceCash">0</span></p><p>银行余额: ¥<span id="balanceBank">0</span></p><p>库存价值: ¥<span id="balanceInventory">0</span></p><p>总资产: ¥<span id="totalAssets">0</span></p><p>会员预收款: ¥<span id="liability">0</span></p><p>所有者权益: ¥<span id="equity">0</span></p></div></div></div></div></div>
<script>
...
<!DOCTYPE html>
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>数据备份与恢复</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet"></head>
<body>
<nav class="navbar navbar-expand-lg navbar-dark bg-dark"><div class="container"><a class="navbar-brand" href="/pos">商超系统</a><div class="navbar-nav"><a class="nav-link" href="/pos">收银台</a><a class="nav-link" href="/inventory">库存管理</a><a class="nav-link" href="/sales">销售记录</a><a class="nav-link" href="/members">会员管理</a><a class="nav-link" href="/purchase">采购入库</a><a class="nav-link" href="/finance">财务期初</a><a class="nav-link active" href="/backup">数据备份</a><a class="nav-link" href="/useradmin">用户管理</a><a class="nav-link" href="/logout">退出</a></div><div class="navbar-nav ms-auto"><button class="btn btn-outline-light btn-sm me-2" onclick="window.print()">🖨️ 打印本页</button></div></div></nav>
